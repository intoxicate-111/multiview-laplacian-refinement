...
| Future2000 direct-vertex Arm E | 已完成 | Job `17888` 于 2026-09-04 完成 200,000 steps（`0:0`，elapsed `2-16:05:51`）。Validation-selected epoch-160 checkpoint SHA-256 为 `5a6aaa32bec6edcdd2c30face02c4ae8bc139fef18d4d05b3394c987057cb50f`；fusion lambda 随后在 validation 上锁定，再一次性打开 test。 |
| Future2000 frozen B+E 对比 | Validation/锁参完成；test 进行中 | Validation array `18673` 已完成全部 1,000 variants，`18677` 未访问 test、按 mean CD `0.00295644415` 选择 `lambda=0.1`。Test shards 0–3 已在 `18678` 中正常完成；原 array 的动态 throttle 使 4–7 停滞，因此它们仅重提一次为最多四卡的 `18780`，12:07 快照时因 Priority 正常排队。Report `18679` 现依赖 `18780_*`。目前不声明 Future2000 Arm-E/B+E test aggregate metric。 |
...
comparison 已完成，invalid external outputs 显式保留。Direct-vertex Arm E 与
B+E validation-only lambda lock 均已完成；one-time test 已部分完成，剩余四个
shards 正在修复后的依赖链中排队。
...
Validation array `18673` 与 lock job `18677` 在打开 test 前完成并选择
`lambda=0.1`。Test shards 0–3 已由 `18678` 完成；原 array 在动态 throttle 处停滞后，
shards 4–7 重提为 `18780`，report `18679` 现依赖 `18780_*`。目前尚不声明
Future2000 Arm-E 或 B+E test aggregate metric。冻结合同、paired samples、invalid

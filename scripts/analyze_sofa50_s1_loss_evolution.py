...
"""Read-only S1 loss-gradient evolution on fixed validation indices."""
...
from analyze_sofa50_loss_mechanisms import _gradient_rows, _rhs_row
from analyze_sofa50_s1_selected import _shared_gradients
...
def main() -> int:
    ...
    for progress, index in enumerate(indices, 1):
        ...
        with torch.enable_grad(), torch.autocast(device_type=device.type, dtype=amp_dtype, enabled=amp_enabled):
            output = model(conditioned)
        direct_prediction = output.direct_vertex_displacement_prediction
        if direct_prediction is None:
            raise RuntimeError("S1 checkpoint has no direct output")
        delta = output.predicted_laplacian.float()
        displacement = direct_prediction.float()
        v_direct = prepared.sample["vertices"].double() + displacement.double()
        recovered, recovery_audit = differentiable_regularized_sparse_recovery_with_audit(
            delta.double(), v_direct,
            prepared.sample["edge_index"], prepared.sample["vertex_degree"].double(),
            regularization=LAMBDA, maximum_iterations=MAXIMUM_ITERATIONS,
            tolerance=TOLERANCE,
        )
        clean_tensor = torch.as_tensor(clean, dtype=torch.float64, device=device)
        loss = (recovered - clean_tensor).square().sum(dim=-1).mean()
        delta_np = delta.detach().double().cpu().numpy()
        direct_np = v_direct.detach().cpu().numpy()
        lap_error = np.linalg.norm(delta_np - delta_gt, axis=1)
        direct_error = np.linalg.norm(direct_np - clean, axis=1)
        initial_mesh = Mesh(vertices, faces.copy()).ensure_normals()
        clean_mesh = _clean_mesh(static)
        geometry = _geometry_row(
            "validation", sample_id, args.label,
            Mesh(recovered.detach().cpu().numpy(), faces.copy()).ensure_normals(),
            clean_mesh, initial_mesh,
        )
        semantic_rows.append({
            "sample_id": sample_id, "sample_index": index,
            "lap_raw_epe": float(lap_error.mean()),
            "direct_vertex_rms": float(np.sqrt(np.mean(np.square(direct_error)))),
            "hybrid_chamfer": float(geometry["chamfer"]),
        })
        sample_shared, sample_audit, gradients = _shared_gradients(
            model, output, loss, delta, displacement
        )
        for row in _gradient_rows("validation", sample_id, faces, gradients, args.chebyshev_order):
            row["checkpoint"] = args.label
            spectral_rows.append(row)
        rhs = _rhs_row(
            "validation", sample_id, "S1", delta.detach().double(),
            torch.as_tensor(delta_gt, dtype=torch.float64, device=device), v_direct.detach(),
            clean_tensor, prepared.sample["edge_index"], prepared.sample["vertex_degree"].double(),
        )
        rhs["checkpoint"] = args.label
        rhs_rows.append(rhs)
        for row in sample_shared:
            row.update({"checkpoint": args.label, "sample_id": sample_id, "sample_index": index})
            shared_rows.append(row)
        sample_audit.update({
            "checkpoint": args.label, "sample_id": sample_id, "sample_index": index,
            "pcg_iterations": int(recovery_audit.iterations),
            "pcg_relative_residual": float(recovery_audit.relative_residual),
            "pcg_converged": bool(recovery_audit.converged),
        })
        audit_rows.append(sample_audit)
        print(f"S1-loss-evolution {args.label} {progress}/{len(indices)} {sample_id}", flush=True)
        if device.type == "cuda":
    ...
    for path in ("g_S1_delta", "g_S1_direct"):
        ...
        spectral_aggregate.append({
            "checkpoint": args.label, "path": path, "samples": len(selected),
    ...
    for layer in sorted({row["layer"] for row in shared_rows}):
        ...
        shared_aggregate.append({
            "checkpoint": args.label, "layer": layer, "samples": len(selected),
    ...
    payload = {
        "read_only": True, "checkpoint": str(args.checkpoint.resolve()),
        "checkpoint_sha256": _sha256(args.checkpoint.resolve()), "label": args.label,
        "indices": indices, "spectral_protocol": SPECTRAL_PROTOCOL,
        "all_finite": all(row["all_finite"] for row in audit_rows),
        "all_pcg_converged": all(row["pcg_converged"] for row in audit_rows),
        ...
        "semantic_aggregate": {
            "lap_raw_epe": float(np.mean([row["lap_raw_epe"] for row in semantic_rows])),
            "direct_vertex_rms": float(np.mean([row["direct_vertex_rms"] for row in semantic_rows])),
            "hybrid_chamfer": float(np.mean([row["hybrid_chamfer"] for row in semantic_rows])),
        },
        "spectral_rows": spectral_rows, "shared_gradient_rows": shared_rows,
        "rhs_rows": rhs_rows, "semantic_rows": semantic_rows,
        "gradient_audits": audit_rows,
    }
    args.output_dir.mkdir(parents=True, exist_ok=True)
    target = args.output_dir / f"s1_loss_evolution_{args.label}.json"
